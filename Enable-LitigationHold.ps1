...
#Logfiles
#########################################################################################################################
$LOGFILENAME = $CurrentDirectory.Path+"\"+$MyDate+"_LitigationHoldEnabled.log"
...
$smtp = "YourSMTPServer"
$subj = "[Job Name] [$MyDate] LitigationHold reports Activations"
$SMTPPorToUsed = "25"
...
#########################################################################################################################
$LitigationHoldToEnabled = Get-Mailbox -ResultSize Unlimited | Where-Object {$_.LitigationHoldEnabled -eq $False -and $_.identity -notlike "DiscoverySearchMailbox*" -and $_.RecipientTypeDetails -notlike "RoomMailbox" -or $_.RecipientTypeDetails -notlike "EquipmentMailbox"}
